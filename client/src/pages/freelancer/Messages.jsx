...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    // Update URL
    if (currentReceiver) {
      navigate(`/freelancer/messages/${currentReceiver._id}`, { replace: true });
    } else {
                  ...
                  {currentReceiver.profileImage ? (
                    ...
                    <div className="h-10 w-10 rounded-full bg-primary-100 flex items-center justify-center">
                      <span className="text-primary-800 font-medium text-sm">
                        {currentReceiver && currentReceiver.name ? currentReceiver.name.charAt(0).toUpperCase() : 'U'}
                      </span>
                    </div>
                  )}
                </div>
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-900">{currentReceiver && currentReceiver.name ? currentReceiver.name : 'User'}</p>
                  <p className="text-xs text-gray-500">
                    {currentReceiver && currentReceiver.isOnline ? 'Online' : 'Offline'}
                  </p>
                </div>
              </div>
